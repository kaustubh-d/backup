...
private
  ...
  def do_get_block(req, proxy, conn, &block)
    ...
    @request_filter.each do |filter|
      if filter.respond_to?(:decrypt_response) and res.status == HTTP::Status::OK
        decrypted_content = filter.decrypt_response(content)
        if content and !content.empty?
          content.replace(decrypted_content)
        end
      end
    end
